...
def redact_note_text(text: str) -> str:
    ...
    Removes:
      ...
      - dates/times
      - long IDs (>=6 digits)
      - any 5-digit sequences (CPT codes & other 5-digit IDs)
    This directly addresses leakage signals (CPT codes in text) and PHI-like patterns.
    """
    if text is None:
        return ""
    ...
    # If any stray MRN/DOB tokens remain (e.g., digits already scrubbed elsewhere), remove them
