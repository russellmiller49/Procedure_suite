...
def test_append_rejects_server_side_scrub_in_strict_mode(client: TestClient, strict_db, monkeypatch) -> None:
    ...
    res = client.post(
        ...
        json={"text": "clinical update", "already_scrubbed": False, "event_type": "clinical_update"},
    )
...
def test_append_absolute_dates_override_blocked_without_env_flag(client: TestClient, strict_db) -> None:
    case_id = uuid.uuid4()
    _seed_case(strict_db, user_id="user_a", registry_uuid=case_id)
