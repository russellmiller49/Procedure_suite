...
def build_seed_path_comparison_report(
    *,
    ...
    metrics = (
        ...
        "strict_render_fallback_rate",
        "forbidden_artifact_case_rate",
        "avg_seed_warning_count",
        "avg_accepted_findings",
        "avg_dropped_findings",
    )
    summary_diff: dict[str, Any] = {}
    left_summary = dict(left_report.get("summary") or {})
    right_summary = dict(right_report.get("summary") or {})
    for metric in metrics:
        left_value = float(left_summary.get(metric, 0.0))
        right_value = float(right_summary.get(metric, 0.0))
        summary_diff[metric] = {
    ...
    left_by_id = {
        ...
        for item in left_report.get("per_case") or []
        if isinstance(item, dict) and item.get("id")
    }
    right_by_id = {
        str(item.get("id") or ""): item
        for item in right_report.get("per_case") or []
        if isinstance(item, dict) and item.get("id")
    }

    rows: list[dict[str, Any]] = []
    critical_mismatch_cases = 0
    right_worse_fallback_cases = 0
    for case_id in sorted(set(left_by_id) | set(right_by_id)):
        left = left_by_id.get(case_id, {})
        right = right_by_id.get(case_id, {})
        left_critical = sorted(left.get("critical_predicted_flags") or [])
        right_critical = sorted(right.get("critical_predicted_flags") or [])
        critical_equal = left_critical == right_critical
        if not critical_equal:
            critical_mismatch_cases += 1

        left_fallback = bool(left.get("render_fallback_used"))
        right_fallback = bool(right.get("render_fallback_used"))
        if right_fallback and not left_fallback:
            right_worse_fallback_cases += 1

        rows.append(
            {
                "id": case_id,
                "left": {
                    "seed_path": left.get("seed_path"),
                    "cpt_jaccard": left.get("cpt_jaccard"),
                ...
                "right": {
                    "seed_path": right.get("seed_path"),
                    "cpt_jaccard": right.get("cpt_jaccard"),
                ...
                "delta": {
                    "cpt_jaccard": round(float(right.get("cpt_jaccard", 0.0)) - float(left.get("cpt_jaccard", 0.0)), 4),
                    "performed_flag_f1": round(
                        float(right.get("performed_flag_f1", 0.0)) - float(left.get("performed_flag_f1", 0.0)),
                        4,
                    ),
                    "seed_warning_count": int(right.get("seed_warning_count", 0)) - int(left.get("seed_warning_count", 0)),
                },
                "critical_presence_equal": critical_equal,
            }
        )

    return {
        "schema_version": REPORTER_SEED_COMPARE_SCHEMA_VERSION,
        ...
        "counts": {
            ...
            "critical_presence_mismatch_cases": critical_mismatch_cases,
            "right_worse_fallback_cases": right_worse_fallback_cases,
        },
        "per_case_diff": rows,
    }
