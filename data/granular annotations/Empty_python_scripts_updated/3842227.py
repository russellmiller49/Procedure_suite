...
entities_1 = [
    ...
    {"label": "MEAS_COUNT", **get_span(text_1, "5", 10)}, # Context "Forceps biopsies: 5"
    
    {"label": "PROC_ACTION", **get_span(text_1, "TBNA passes", 1)},
    {"label": "MEAS_COUNT", **get_span(text_1, "4", 15)}, # Context "TBNA passes: 4"
    ...
    {"label": "OUTCOME_COMPLICATION", **get_span(text_1, "None", 2)}, # Context: "COMPLICATIONS: None" (First "None" is Fellow PGY-None usually, check index. Actually first None is in complications)
    # Checking "None" occurrences:
    # 1. PGY-None (Bottom of text)
    # 2. COMPLICATIONS: None (Middle)
    # Wait, scanning text:
    # "COMPLICATIONS: None" is near bottom.
    # "Fellow, PGY-None" is at very bottom.
    # Let's verify text order. "COMPLICATIONS" comes before "Fellow". So it is occurrence 1.
    
    {"label": "MEAS_VOL", **get_span(text_1, "<10 mL", 1)},
]

# Correction on "None" logic:
# COMPLICATIONS: None is found BEFORE "PGY-None". 
# So usage below is corrected to occurrence 1.
entities_1 = [e for e in entities_1 if e['label'] != "OUTCOME_COMPLICATION"] + [{"label": "OUTCOME_COMPLICATION", **get_span(text_1, "None", 1)}]
