...
procedure flags from clinical procedure notes. Key features:
...
class RoBERTaPM3MultiLabel(nn.Module):
    """RoBERTa-PM-M3-Voc-distill with multi-label classification head.
    ...
    @classmethod
    def from_pretrained(cls, path: Path, num_labels: int) -> "RoBERTaPM3MultiLabel":
...
def train(config: TrainingConfig) -> dict[str, Any]:
    ...
    """
    print(f"\n{'=' * 60}")
    print("RoBERTa PM3 Registry Classifier Training")
    print(f"{'=' * 60}")
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/models/roberta_pm3_registry"),
    ...
    parser.add_argument(
        "--model-name",
        type=str,
        default="data/models/RoBERTa-base-PM-M3-Voc-distill/RoBERTa-base-PM-M3-Voc-distill-hf",
        help="Local RoBERTa-PM-M3-Voc-distill HuggingFace path",
    )
