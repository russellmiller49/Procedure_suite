#!/usr/bin/env python3
"""
Parse 'reporter examples.txt' into a structured JSON dataset for testing.

Usage:
    python scripts/parse_golden_reporter_examples.py \
        --input "reporter examples.txt" \
...
def main():
    ...
    # with no args (it will default to `reporter examples.txt` in the repo root).
    parser.add_argument(
        "input",
        nargs="?",
        type=Path,
        default=Path("reporter examples.txt"),
        help='Path to raw txt file (default: "reporter examples.txt")',
    )
