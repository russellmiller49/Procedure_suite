...
**Key Files:**
- `fastapi_app.py` - Composition root (app creation, middleware, static mounts, router wiring)
- `bootstrap.py` - Startup/shutdown lifecycle orchestration (`StartupBootstrap`)
- `routes/` - Endpoint handlers grouped by domain (`legacy_coder.py`, `legacy_registry.py`, `reporting.py`, `qa.py`, etc.)
...
| `/report/render` | POST | Generate synoptic report |
...
app/registry/
├── application/
│   ├── registry_service.py     # Orchestrator (extraction-first primary; legacy hybrid isolated)
│   ├── registry_builder.py     # Build registry entries
│   └── cpt_registry_mapping.py # CPT → registry field mapping
├── heuristics/
│   ├── navigation_targets.py   # Target lesion / navigation heuristics
│   ├── linear_ebus_station_detail.py
│   ├── cao_detail.py
│   ├── coverage_checks.py
│   └── granular_warning_reconcile.py
├── infra/
│   └── model_provider.py       # Backend-aware ML predictor selection (Torch/ONNX/sklearn)
├── constants/
│   ├── field_paths.py          # High-risk registry path literals
│   └── warnings.py             # Shared warning message helpers
├── adapters/
...
**Target: Extraction-First Registry Flow (production)**
...
Startup invariant enforcement now lives in `config/startup_settings.py` and is invoked by `app/api/bootstrap.py`.
...
### Dependency Workflow
- Direct runtime inputs live in `requirements.in`.
- Pinned/transitive lock lives in `requirements.txt` (generated by pip-tools).
- Regenerate with `make deps-compile`.
- Verify sync with `make deps-check` (also enforced in CI).
- Lock generation targets CPython 3.11 + `manylinux2014_x86_64` for deterministic CI/prod resolution.
