...
The examples below assume `data/knowledge/ip_coding_billing_v2_9.json` as the current baseline. The updater will increment the minor version automatically.

```bash
# Update your JSON with the included 2025 RVU data
python local_rvu_updater.py update \
    --input data/knowledge/ip_coding_billing_v2_9.json \
    --rvu-csv cms_rvus_2025_ip.csv \
    --output data/knowledge/ip_coding_billing_v2_9.json \
...
After running the updater, your JSON will have a new `cms_rvus` section. As of December 2025, these examples assume `data/knowledge/ip_coding_billing_v2_9.json` with a `cms_rvus` block structured like the following:
...
with open('data/knowledge/ip_coding_billing_v2_9.json') as f:
